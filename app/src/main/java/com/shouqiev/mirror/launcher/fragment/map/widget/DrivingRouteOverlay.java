...
public class DrivingRouteOverlay extends RouteOverlay {
  ...
  private int getcolor(String status) {
    ...
    if (status.equals("畅通")) {
      return Color.parseColor("#76D085");
    } else if (status.equals("缓行")) {
      return Color.parseColor("#F7D15F");
    } else if (status.equals("拥堵")) {
      return Color.parseColor("#D33024");
  ...
  @Override protected LatLngBounds getLatLngBounds() {
  ...
  private void addThroughPointMarker() {
    if (this.throughPointList != null && this.throughPointList.size() > 0) {
      ...
      for (int i = 0; i < this.throughPointList.size(); i++) {
        ...
        if (latLonPoint != null) {
          throughPointMarkerList.add(mAMap.addMarker(
              (new MarkerOptions()).position(new LatLng(latLonPoint.getLatitude(), latLonPoint.getLongitude()))
